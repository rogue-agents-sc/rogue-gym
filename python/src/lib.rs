...
impl GameState {
    #[new]
    fn __new__(max_steps: usize, config_str: Option<String>) -> PyResult<GameState> {
...
impl ParallelGameState {
    #[new]
    fn __new__(py: Python, max_steps: usize, configs: Vec<String>) -> PyResult<ParallelGameState> {
        let configs = {
            let mut res = vec![];
            for cfg in configs {
